...
Only `src/index.js` is published as the library; `.claspignore` keeps `main.partial.js` out.

```bash
npm i -g @google/clasp
clasp login
clasp create --type standalone --title "CrUX Extractor" --rootDir src
clasp push                              # pushes src/index.js only
clasp version "v1 - initial release"    # cut a version consumers can pin
```

Then share the script (Project Settings share, or "Anyone with the link – Viewer") and give consumers the **Script ID** (Project Settings > IDs). Cut a new `clasp version` on each release. See [Libraries guide](https://developers.google.com/apps-script/guides/libraries).
